...
int main(int argc, char **argv) { 
	...
	try {
		...
		// getValue(const std::string& parameter, const std::string& default_value)
		// 
		Data train(
		...
		fm_model fm;
		{
			...
			{ 
				...
				fm.k0 = dim[0] != 0;
				fm.k1 = dim[1] != 0;
				// num_factor = k, k is the column number of the low rank Matrix V of W matrix
				fm.num_factor = dim[2];					
			}			
			fm.init();		
		}
		...
		// fml is the wrapper of the fm model for the total libFM
		// set the initialnization of different learning model according to the method parameter user's provided
		fm_learn* fml;
